...
{
    public class CriticalTaskViewModel : BaseViewModel, ITodoTask
    {
        ...
        public void FinishTask(TodoTask task)
        {
            task.Type = task.IsFinished ?
                TaskType.Success : TaskType.Critical;

            task.IconData = task.IsFinished ?
                _geometryData.Success : _geometryData.Critical;
        ...
        public async Task ReadTasksAsync(string taskPath)
        {
            ...
            {
                if (todoTask.Type != TaskType.Critical) continue;
                todoTask.IconData = _geometryData.Critical;
        ...
        public void AddTask(TodoTask task)
        {
            ...
            task.IconData = _geometryData.Critical;
            task.Type = TaskType.Critical;
            task.Title = Title;
            TodoTasks.Add(task);
            ToDoTask = new TodoTask { CanBeFinished = true };
        }
