...
{
    public class WarningTaskViewModel : BaseViewModel, ITodoTask
    {
        #region Declarations
        ...
        public WarningTaskViewModel(ITaskService taskService)
        {
        ...
        public void FinishTask(TodoTask task)
        {
            task.Type = task.IsFinished ?
                TaskType.Success : TaskType.Warning;

            task.IconData = task.IsFinished ?
                _geometryData.Success : _geometryData.Warning;
        ...
        public async Task ReadTasksAsync(string taskPath)
        {
            ...
            {
                if (todoTask.Type != TaskType.Warning) continue;
                todoTask.IconData = _geometryData.Warning;
        ...
        public void AddTask(TodoTask task)
        {
            ...
            task.IconData = _geometryData.Warning;
            task.Type = TaskType.Warning;
            task.Title = Title;
            TodoTasks.Add(task);
            ToDoTask = new TodoTask { CanBeFinished = true };
        }
